...
parser.add_argument('-nt', type=int, default=None, help='Total number of data points')
...
np.savetxt('test.idxs', np.sort(testIdxs), fmt='%d')

foldSize = int(float(len(randomIdxs))/args.k)

# Fill the validation and train sets
f = open('kValidate.idxs', 'w')
g = open('kTrain.idxs', 'w')
n = 0
